...
export const zh = {
  ...
  pickElement: '选择网页元素加入聊天',
  pickElementCancel: '取消网页元素选择',
  pickElementHint: '点击页面中的元素加入聊天，Esc 取消',
  pickElementUnsupported: '当前页面不支持选择元素：只有本插件提供的 HTML 预览页面内置了选择器，跨源站点无法注入',
  pickerLabelColor: '颜色',
  pickerLabelBackground: '背景',
  pickerLabelFont: '字体',
  pickElementViaProxy: '通过内置代理加载后选择元素（远端站点无法直接注入）',
  pickElementProxyFailed: '无法开启代理加载，请重试',
  browserProxyNotice: '当前页面由内置代理重新加载，以便选择元素；脚本、登录态与部分资源可能失效',
  browserProxyRestore: '恢复直连',
  browserProxyStatic: '静态模式重载',
  browserProxyStaticNotice: '静态模式：已去掉页面自身脚本（跳转/黑屏页面用此模式仍可选择元素）',
  subagent: '任务管理',
  openSubagent: '任务管理',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  pickElement: 'Select page element for chat',
  pickElementCancel: 'Cancel element selection',
  pickElementHint: 'Click an element to add it to the chat — Esc cancels',
  pickElementUnsupported: 'Element selection is unavailable here: only the HTML previews this plugin serves carry the picker, and a cross-origin site cannot be injected',
  pickerLabelColor: 'Color',
  pickerLabelBackground: 'Background',
  pickerLabelFont: 'Font',
  pickElementViaProxy: 'Reload through the built-in proxy to select an element (a remote site cannot be injected directly)',
  pickElementProxyFailed: 'Could not start the proxy load — try again',
  browserProxyNotice: 'This page was re-served by the built-in proxy so elements can be selected; scripts, logins and some resources may not work',
  browserProxyRestore: 'Load directly',
  browserProxyStatic: 'Reload without page scripts',
  browserProxyStaticNotice: 'Static mode: the page\'s own scripts are stripped, so a redirecting or blank page stays pickable',
  subagent: 'Tasks',
  openSubagent: 'Tasks',
